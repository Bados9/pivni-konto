...
# Web Push needs VAPID keys in backend/.env (or .env.local) - gitignored, hand-maintained on the server
echo "🔔 Checking VAPID keys..."
if ! docker compose -f docker-compose.prod.yml exec -T php sh -c 'grep -qs "^VAPID_PUBLIC_KEY=." .env .env.local'; then
    echo "⚠️  VAPID keys missing in backend/.env - Web Push notifications will NOT work."
    echo "   Generate them: docker compose -f docker-compose.prod.yml exec php php bin/console app:generate-vapid-keys"
    echo "   Add the two lines to backend/.env and restart the php and cron containers."
fi
