...
#[AsCommand(
    name: 'app:load-beers',
    description: 'Load beer data into database (safe to re-run, skips existing)',
)]
class LoadBeersCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->success(sprintf('Loaded %d new beers (skipped %d existing)', $added, count($beers) - $added));

        return Command::SUCCESS;
    }

    private function getBeerData(): array
    {
        return [
            // === VELKÉ PIVOVARY ===
