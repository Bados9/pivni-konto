...
class AchievementService
{
    private array $achievementDefinitions = [
        ...
        'night_owl' => [
            'name' => 'Noční sova',
            'description' => 'Vypij pivo mezi 22:00 a 2:00',
            'icon' => '🦉',
            'category' => 'time',
            'repeatable' => true,
        ],
        ...
        'streak_14' => [
            'name' => 'Železná játra',
            'description' => 'Pij pivo 14 dní v řadě',
            'icon' => '💪',
            'category' => 'performance',
        ],
        'streak_30' => [
            'name' => 'Maratonec',
            'description' => 'Pij pivo 30 dní v řadě',
            'icon' => '🏃',
            'category' => 'performance',
        ],
        ...
        'new_year' => [
            'name' => 'Silvestrovský přípitek',
            'description' => 'Dej si pivo na Silvestra',
            'icon' => '🎆',
            'category' => 'special',
            'repeatable' => true,
        ],
    ...
    private function getRepeatableCount(string $id, array $stats): int
    {
        return match ($id) {
            ...
            'night_owl' => $stats['night_owl_days'] ?? 0,
            'taster_day' => $stats['tasting_days'] ?? 0,
            'new_year' => $stats['new_year_days'] ?? 0,
    ...
    private function calculateUserStats(User $user): array
    {
        ...
        return [
            ...
            'night_owl_days' => $dbStats['night_owl_days'],
            'new_year_days' => $dbStats['new_year_days'],
    ...
    private function isAchievementUnlocked(string $id, array $stats): bool
    {
        return match ($id) {
            ...
            'night_owl' => ($stats['night_owl_days'] ?? 0) >= 1,
            ...
            'streak_14' => $stats['consecutive_days'] >= 14,
            'streak_30' => $stats['consecutive_days'] >= 30,
            ...
            'new_year' => ($stats['new_year_days'] ?? 0) >= 1,
    ...
    private function getAchievementProgress(string $id, array $stats): array
    {
        return match ($id) {
            ...
            'night_owl' => ['current' => min($stats['night_owl_days'] ?? 0, 1), 'target' => 1],
            ...
            'streak_14' => ['current' => min($stats['consecutive_days'], 14), 'target' => 14],
            'streak_30' => ['current' => min($stats['consecutive_days'], 30), 'target' => 30],
            ...
            'new_year' => ['current' => min($stats['new_year_days'] ?? 0, 1), 'target' => 1],
